...
export default function SearchBar() {
  ...
  return (
    <div className="relative max-w-2xl mx-auto z-10 px-4 sm:px-6">
      <div className="flex items-center gap-3 bg-white dark:bg-[#161b22] border border-slate-300 dark:border-[#30363d] rounded-full shadow-card pl-5 pr-2 py-2 focus-within:border-slate-900 dark:focus-within:border-slate-100 transition">
      ...
      {open && q.trim().length >= 2 && (
        <div className="absolute left-4 right-4 sm:left-6 sm:right-6 top-full mt-2 bg-white dark:bg-[#161b22] border border-slate-200 dark:border-[#30363d] rounded-2xl shadow-pop overflow-hidden max-h-80 overflow-y-auto">
          {hits.length ? hits.map((h) => (
            <Link key={`${h.s}-${h.g}-${h.c}`} href={`/chapter/${h.s}/${h.g}/${h.c}`}
              ...
              <span className="w-9 h-9 rounded-lg grid place-items-center bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-slate-300 flex-none">
