...
export default function SubjectGrid() {
 ...
 className="reveal group card-g p-6 hover:shadow-lift hover:-translate-y-1 hover:border-slate-300 transition-all duration-200">
 <div className="flex items-start justify-between">
 <span className="w-12 h-12 rounded-xl grid place-items-center bg-slate-100 text-slate-700 ">
 <Icon name={s.icon} size={24} />
 </span>
 <span className="font-mono text-[12px] font-medium text-ink-mute ">{n} chapters</span>
 </div>
 <h3 className="font-extrabold text-[20px] tracking-tight mt-4">{s.name}</h3>
 <p className="text-[13.5px] text-ink-mute mt-1.5 leading-relaxed min-h-[42px]">{s.tagline}</p>
 <div className="flex items-center justify-between text-[12.5px] font-semibold mt-4">
 <span className="font-mono font-medium text-ink-mute ">{pct}% complete</span>
