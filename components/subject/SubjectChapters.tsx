...
function Card({ s, gi, ci, ch }: { s: Subject; gi: number; ci: number; ch: ChapterRef }) {
  ...
  return (
    <div className={`reveal card-g p-4 flex gap-3.5 items-start hover:shadow-lift hover:-translate-y-0.5 transition ${done ? "ring-2 ring-g-green/60" : ""}`}>
      <Link href={`/chapter/${s.id}/${gi}/${ci}`} aria-label={ch.title}
        className="w-12 h-12 rounded-2xl grid place-items-center font-display font-extrabold text-[19px] flex-none"
        style={{ background: `${s.color}1a`, color: s.color }}>
        {ch.n}
      </Link>
      <div className="min-w-0 flex-1">
        <Link href={`/chapter/${s.id}/${gi}/${ci}`} className="font-bold text-[15.5px] leading-snug block hover:text-brand-600 dark:hover:text-brand-300 transition">
          {ch.title}
        </Link>
        {ch.hi && <p className="text-[13px] text-ink-mute dark:text-slate-400 mt-0.5 truncate">{ch.hi}</p>}
        <div className="flex gap-1.5 mt-2 flex-wrap">
          {full
            ? <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-full bg-g-green/15 text-g-green"><Icon name="star" size={12} /> Full content</span>
            : <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-full bg-g-amber/20 text-[#7a4a00] dark:text-g-amber"><Icon name="info" size={12} /> Auto guide</span>}
      ...
      <button
        ...
        className={`w-10 h-10 rounded-full grid place-items-center flex-none border-2 transition ${done
          ? "bg-g-green border-g-green text-white"
          : "border-[#e1e3e6] dark:border-[#444746] text-transparent hover:border-g-green"}`}
      >
    ...
    <div key={tick}>
      ...
      <div className="card-g p-4 sm:p-5 mb-7">
        <div className="flex items-center gap-4 flex-wrap">
          <span className="w-12 h-12 rounded-2xl grid place-items-center text-white flex-none" style={{ background: sub.color }}>
            <Icon name="barChart" size={24} />
          </span>
          <div className="flex-1 min-w-[180px]">
            <div className="flex items-baseline justify-between gap-2">
              <b className="font-display text-[16px]">Syllabus progress</b>
              <span className="text-[13px] font-bold text-ink-mute dark:text-slate-400">{doneCount} of {total} · {pct}%</span>
            </div>
            <div className="h-2.5 rounded-full bg-slate-100 dark:bg-white/10 mt-2 overflow-hidden">
        ...
        <div className="flex gap-2 mt-3.5 flex-wrap">
          <Link href="/smart-board" className="btn-g btn-g-white text-[13.5px]">
            <Icon name="squarePen" size={17} /> Smart Board kholo
          </Link>
          ...
          <Link href="/admin" className="btn-g btn-g-white text-[13.5px]">
            <Icon name="shieldCheck" size={17} /> Teacher? Edit content
          </Link>
        </div>
      </div>
      ...
      {sub.groups.map((g, gi) => (
        <section key={g.label} className="mb-8">
          <div className="flex items-center gap-2.5 mb-3.5">
            <h2 className="font-display text-[19px] sm:text-[21px] font-extrabold tracking-tight">{g.label}</h2>
            <span className="inline-flex items-center gap-1 text-[12px] font-bold px-3 py-1 rounded-full text-white" style={{ background: sub.color }}>
