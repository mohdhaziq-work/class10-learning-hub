...
function Card({ s, gi, ci, ch }: { s: Subject; gi: number; ci: number; ch: ChapterRef }) {
 ...
 className="w-12 h-12 rounded-xl grid place-items-center font-extrabold text-[19px] flex-none bg-slate-100 text-slate-800 ">
 {ch.n}
 </Link>
 ...
 {ch.hi && <p className="text-[13px] text-ink-mute mt-0.5 truncate">{ch.hi}</p>}
 ...
 : "border-slate-200 text-transparent hover:border-slate-900 "}`}
 >
...
export default function SubjectChapters({ sub }: { sub: Subject }) {
 ...
 <span className="w-12 h-12 rounded-xl grid place-items-center text-white bg-slate-900 flex-none">
 <Icon name="barChart" size={24} />
 </span>
 <div className="flex-1 min-w-[180px]">
 <div className="flex items-baseline justify-between gap-2">
 <b className="text-[16px] tracking-tight">Syllabus progress</b>
 <span className="font-mono text-[12.5px] text-ink-mute ">{doneCount} of {total} · {pct}%</span>
 </div>
 ...
 <span className="inline-flex items-center gap-1 text-[12px] font-bold px-3 py-1 rounded-full bg-slate-900 text-white ">
