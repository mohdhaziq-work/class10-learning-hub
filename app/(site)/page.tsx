...
function SectionHead({ overline, title, sub }: { overline: string; title: string; sub: string }) {
  return (
    <div className="max-w-2xl mb-8">
      ...
      <p className="text-ink-soft dark:text-slate-400 mt-2.5 text-[15.5px] leading-relaxed">{sub}</p>
    </div>
  );
}
...
export default function HomePage() {
  return (
    <div>
      {/* ---------- HERO ---------- */}
      <section className="border-b border-slate-200 dark:border-[#30363d]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-14 sm:pt-20 pb-12 grid gap-12 lg:grid-cols-[1.05fr_.95fr] items-center">
          <div>
            <span className="inline-flex items-center gap-2 text-[12px] font-mono font-medium px-4 py-2 rounded-full bg-white dark:bg-[#161b22] border border-slate-200 dark:border-[#30363d] text-ink-soft dark:text-slate-300">
            ...
            <p className="text-ink-soft dark:text-slate-400 text-[16.5px] sm:text-lg leading-relaxed mt-5 max-w-xl">
            ...
            <dl className="grid grid-cols-2 sm:grid-cols-4 gap-6 mt-10 max-w-xl">
              {[
                ...
                <div key={s.l} className="border-t-2 border-slate-900 dark:border-slate-100 pt-3">
                  <dd className="text-[30px] font-extrabold tracking-tight leading-none">{s.v}</dd>
                  <dt className="text-[12.5px] font-medium text-ink-mute dark:text-slate-400 mt-1.5">{s.l}</dt>
                </div>
              ))}
            </dl>
          </div>
          ...
          <div className="relative hidden md:block">
            <div className="rounded-3xl overflow-hidden bg-white dark:bg-[#161b22] border border-slate-200 dark:border-[#30363d] shadow-lift">
              <div className="flex items-center gap-2 px-5 py-3.5 border-b border-slate-200 dark:border-[#30363d]">
                <span className="w-2.5 h-2.5 rounded-full bg-slate-300 dark:bg-slate-600" />
                <span className="w-2.5 h-2.5 rounded-full bg-slate-300 dark:bg-slate-600" />
                <span className="w-2.5 h-2.5 rounded-full bg-slate-300 dark:bg-slate-600" />
                <span className="ml-2 font-mono text-[11.5px] font-medium text-ink-mute dark:text-slate-400 tracking-wider">SMART BOARD — LIVE</span>
              </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {STEPS.map((s) => (
              <div key={s.n} className="reveal card-g p-6">
                <p className="font-mono text-[13px] font-semibold text-ink-mute dark:text-slate-500">{s.n}</p>
                <h3 className="font-bold text-[16.5px] tracking-tight mt-2.5">{s.t}</h3>
                <p className="text-[13.5px] text-ink-mute dark:text-slate-400 mt-1.5 leading-relaxed">{s.d}</p>
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {FEATURES.map((f) => (
              <div key={f.t} className="reveal card-g p-6 flex gap-4 hover:shadow-lift hover:-translate-y-0.5 transition">
                <span className="w-11 h-11 rounded-xl grid place-items-center bg-slate-100 dark:bg-white/10 text-slate-700 dark:text-slate-200 flex-none">
        ...
        {/* ---------- CTA ---------- */}
        <section className="my-16 rounded-3xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 p-8 sm:p-12 text-center">
          <p className="font-mono text-[11.5px] font-medium uppercase tracking-[.18em] text-slate-400 dark:text-slate-500">Get started</p>
          <h2 className="text-[26px] sm:text-[34px] font-extrabold tracking-tight mt-3">Ready for smarter classes?</h2>
          <p className="text-slate-400 dark:text-slate-500 mt-2 text-[15.5px]">Open a chapter, or launch the board — it takes five seconds.</p>
          <div className="flex gap-3 justify-center mt-7 flex-wrap">
            <Link href="/subjects/maths" className="btn-g bg-white text-slate-900 hover:bg-slate-200 dark:bg-slate-900 dark:text-white dark:hover:bg-black text-[15px]">
              <Icon name="bookOpen" size={18} /> Start with Maths
            </Link>
            <Link href="/smart-board" className="btn-g text-[15px] border border-white/30 dark:border-slate-900/30 hover:bg-white/10 dark:hover:bg-black/5">
              <Icon name="squarePen" size={18} /> Open Smart Board
            </Link>
          </div>
        </section>
